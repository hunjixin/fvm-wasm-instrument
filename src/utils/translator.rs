...
pub fn op(t: &dyn Translator, op: &Operator<'_>) -> Result<Instruction<'static>> {
	...
	Ok(match op {
		...
		O::MemoryInit { segment, mem } => I::MemoryInit { data: *segment, mem: *mem },
		O::DataDrop { segment } => I::DataDrop(*segment),
		O::MemoryCopy { src, dst } => I::MemoryCopy { src: *src, dst: *dst },
		O::MemoryFill { mem, .. } => I::MemoryFill(*mem),

		O::TableInit { segment, table } => I::TableInit { segment: *segment, table: *table },
		O::ElemDrop { segment } => I::ElemDrop { segment: *segment },
		O::TableCopy { dst_table, src_table } => I::TableCopy { dst: *dst_table, src: *src_table },
		...
		O::V128Load { memarg } => I::V128Load { memarg: t.translate_memarg(memarg)? },
		O::V128Load8x8S { memarg } => I::V128Load8x8S { memarg: t.translate_memarg(memarg)? },
		O::V128Load8x8U { memarg } => I::V128Load8x8U { memarg: t.translate_memarg(memarg)? },
		O::V128Load16x4S { memarg } => I::V128Load16x4S { memarg: t.translate_memarg(memarg)? },
		O::V128Load16x4U { memarg } => I::V128Load16x4U { memarg: t.translate_memarg(memarg)? },
		O::V128Load32x2S { memarg } => I::V128Load32x2S { memarg: t.translate_memarg(memarg)? },
		O::V128Load32x2U { memarg } => I::V128Load32x2U { memarg: t.translate_memarg(memarg)? },
		O::V128Load8Splat { memarg } => I::V128Load8Splat { memarg: t.translate_memarg(memarg)? },
		O::V128Load16Splat { memarg } => I::V128Load16Splat { memarg: t.translate_memarg(memarg)? },
		O::V128Load32Splat { memarg } => I::V128Load32Splat { memarg: t.translate_memarg(memarg)? },
		O::V128Load64Splat { memarg } => I::V128Load64Splat { memarg: t.translate_memarg(memarg)? },
		O::V128Load32Zero { memarg } => I::V128Load32Zero { memarg: t.translate_memarg(memarg)? },
		O::V128Load64Zero { memarg } => I::V128Load64Zero { memarg: t.translate_memarg(memarg)? },
		O::V128Store { memarg } => I::V128Store { memarg: t.translate_memarg(memarg)? },
